...
void LoadConfig()
{
    ...
    GConfig.bLogLogin6C = GetPrivateProfileIntA("Logging", "LogLogin6C", 1, IniPath) != 0;
    GConfig.bHookLogin6DHandler = GetPrivateProfileIntA("Logging", "HookLogin6DHandler", 0, IniPath) != 0;
    GConfig.bHookLogin6DReturn = GetPrivateProfileIntA("Logging", "HookLogin6DReturn", 0, IniPath) != 0;
    GConfig.bHookLoginSerialize = GetPrivateProfileIntA("Logging", "HookLoginSerialize", 0, IniPath) != 0;
    GConfig.bHookLoginSend = GetPrivateProfileIntA("Logging", "HookLoginSend", 0, IniPath) != 0;
    GConfig.bClientLog6D = GetPrivateProfileIntA("Logging", "ClientLog6D", 0, IniPath) != 0;
    ...
    char Login6DModule[MAX_PATH] = {0};
    GetPrivateProfileStringA("Logging", "Login6DHandlerModule", "", Login6DModule, MAX_PATH, IniPath);
    if (Login6DModule[0])
    {
        lstrcpynA(GConfig.Login6DHandlerModule, Login6DModule, MAX_PATH);
    }
    char Login6DRvaText[64] = {0};
    GetPrivateProfileStringA("Logging", "Login6DHandlerRva", "", Login6DRvaText, sizeof(Login6DRvaText), IniPath);
    if (Login6DRvaText[0])
    {
        char* End = nullptr;
        unsigned long Parsed = strtoul(Login6DRvaText, &End, 0);
        if (End && End != Login6DRvaText)
        {
            GConfig.Login6DHandlerRva = static_cast<uint32_t>(Parsed);
        }
    }

    char LoginSerializeRvaText[64] = {0};
    GetPrivateProfileStringA("Logging", "LoginSerializeRva", "", LoginSerializeRvaText, sizeof(LoginSerializeRvaText), IniPath);
    if (LoginSerializeRvaText[0])
    {
        char* End = nullptr;
        unsigned long Parsed = strtoul(LoginSerializeRvaText, &End, 0);
        if (End && End != LoginSerializeRvaText)
        {
            GConfig.LoginSerializeRva = static_cast<uint32_t>(Parsed);
        }
    }

    char LoginSendRvaText[64] = {0};
    GetPrivateProfileStringA("Logging", "LoginSendRva", "", LoginSendRvaText, sizeof(LoginSendRvaText), IniPath);
    if (LoginSendRvaText[0])
    {
        char* End = nullptr;
        unsigned long Parsed = strtoul(LoginSendRvaText, &End, 0);
        if (End && End != LoginSendRvaText)
        {
            GConfig.LoginSendRva = static_cast<uint32_t>(Parsed);
        }
    }
    ...
    GetPrivateProfileStringA("Huffman", "TablePath", "..\\ServerEmulator\\huffman_table_runtime.json", HuffmanPath, MAX_PATH, IniPath);
    if (HuffmanPath[0])
    {
