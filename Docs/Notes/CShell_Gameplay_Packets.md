...
Scope: server->client packets decoded from CShell.dll (FoM). This is a starting point for emulator-side
structs and parsers; exact offsets and helper names are in `AddressMap.md`.
...
- ItemsAdded payload: u16c baseUsedCount + 3x u32c (capacity/unk24/unk28) + u16c entryCount + [ItemStructA + u16c variantIdCount + variantIdCount*u32c].
- list_u16count_u32c: u16c count, then count*u32c.
...
- Only RTTI/vftable list reference found (no handlers or packet dispatch use in FoM binaries).
