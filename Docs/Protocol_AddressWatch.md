# Protocol Address Watchlist (FoM target)

Source: `Docs\Notes\` (legacy build notes). These VAs are **not validated** for `FoM`; re-locate in `FoM` via RTTI strings/xrefs.

## Server/FoM Flow Anchors
