...
export class FomReliabilityLayer {
    ...
    parseReliablePacket(data: Buffer): ParsedReliablePacket | null {
        ...
        console.log(
            `  Inner data: ${innerData.length} bytes, ID: 0x${innerMessageId.toString(16)}`,
        );
    ...
    sendReliable(data: Buffer): void {
        ...
        console.log(
            `[FomRL] Sending reliable #${this.messageNumberOut}: ${packet.toString('hex')}`,
        );
    ...
    private sendPendingAcks(): void {
        ...
        console.log(
            `[FomRL] Sending ACK (byte-aligned): ${ack.toString('hex')} for msg ${msgToAck}`,
        );
