...
export function loadEnvCandidates(): void {
    ...
    const explicit = process.env.FOM_RSA_ENV || process.env.FOM_KEY_ENV;
    if (explicit) envCandidates.push(explicit);

    const cwd = process.cwd();
    envCandidates.push(path.resolve(cwd, 'fom_private_key.env'));
    envCandidates.push(path.resolve(cwd, '.env'));
    envCandidates.push(path.resolve(cwd, 'ServerEmulator', '.env'));
    envCandidates.push(path.resolve(cwd, 'ServerEmulator', 'fom_private_key.env'));
    envCandidates.push(path.resolve(cwd, '..', 'ServerEmulator', 'fom_private_key.env'));
    ...
    for (const candidate of envCandidates) {
        try {
            ...
            if (applied > 0) {
                ...
                console.log(`[FoMServer] Loaded ${applied} env vars from ${candidate}`);
                break;
            }
        } catch {
    ...
    iniCandidates.push(path.resolve(cwd, 'fom_server.ini'));
    iniCandidates.push(path.resolve(cwd, 'server.ini'));
    iniCandidates.push(path.resolve(cwd, 'config.ini'));
    iniCandidates.push(path.resolve(cwd, 'ServerEmulator', 'fom_server.ini'));
    iniCandidates.push(path.resolve(cwd, 'ServerEmulator', 'server.ini'));
    iniCandidates.push(path.resolve(cwd, 'ServerEmulator', 'config.ini'));
    iniCandidates.push(path.resolve(cwd, '..', 'ServerEmulator', 'fom_server.ini'));
    ...
    for (const candidate of iniCandidates) {
        try {
            ...
            if (applied > 0) {
                // eslint-disable-next-line no-console
                console.log(`[FoMServer] Loaded ${applied} ini vars from ${candidate}`);
                break;
            }
        } catch {
...
export function cleanupLegacyLogs(): void {
    ...
    const candidates = [
        ...
        path.resolve(cwd, 'ServerEmulator', 'logs', 'trace_raw.log'),
        path.resolve(cwd, 'ServerEmulator', 'logs', 'lithdebug.log'),
    ];
...
export function parseU32(value: string | undefined, defaultValue: number): number {
...
function resolveConfigOutputPath(): string {
    ...
    const cwd = process.cwd();
    if (path.basename(cwd).toLowerCase() === 'serveremulator') {
        return path.resolve(cwd, 'fom_server.ini');
    }
    return path.resolve(cwd, 'ServerEmulator', 'fom_server.ini');
}
...
export function writeConfigSnapshot(): void {
    ...
    const worldSelectWorldId = parseU32(process.env.FOM_WORLD_ID || process.env.WORLD_ID, 16);
    const worldSelectWorldInst = parseU32(process.env.FOM_WORLD_INST || process.env.WORLD_INST, 0);
    const worldSelectPlayerId = parseU32(process.env.FOM_PLAYER_ID || process.env.PLAYER_ID, 0);
    const worldSelectPlayerIdRandom = parseBool(
        process.env.FOM_PLAYER_ID_RANDOM || process.env.PLAYER_ID_RANDOM,
        worldSelectPlayerId === 0,
    );
    const worldLoginWorldConst = parseU32(
    ...
    const loginBlobBytes =
        blobBytesRaw > 0 ? blobBytesRaw : blobBitsRaw > 0 ? Math.ceil(blobBitsRaw / 8) : 256;

    const lines: string[] = [];
    lines.push('# FoM Server Emulator config');
    ...
    lines.push(`SERVER_MODE=${process.env.SERVER_MODE || 'master'}`);
    ...
    lines.push(
        `FOM_CLEAN_LEGACY_LOGS=${formatBool(parseBool(process.env.FOM_CLEAN_LEGACY_LOGS, false))}`,
    );
    lines.push(
        `PACKET_HANDLER_VERBOSE=${formatBool(parseBool(process.env.PACKET_HANDLER_VERBOSE, false))}`,
    );
    lines.push(`LOGIN_DEBUG=${formatBool(parseBool(process.env.LOGIN_DEBUG, false))}`);
    lines.push(
        `PACKET_HANDLER_LOG_THROTTLE_MS=${parseInt(process.env.PACKET_HANDLER_LOG_THROTTLE_MS || '5000', 10) || 0}`,
    );
    lines.push('');
    ...
    lines.push(
        `LITH_DEBUG_HEX_BYTES=${parseInt(process.env.LITH_DEBUG_HEX_BYTES || '48', 10) || 0}`,
    );
    lines.push(`LITH_DEBUG_RAW=${formatBool(parseBool(process.env.LITH_DEBUG_RAW, false))}`);
    lines.push(
        `LITH_DEBUG_RAW_BYTES=${parseInt(process.env.LITH_DEBUG_RAW_BYTES || '512', 10) || 0}`,
    );
    ...
    lines.push(
        `LITH_DEBUG_SCAN_ANY=${formatBool(parseBool(process.env.LITH_DEBUG_SCAN_ANY, false))}`,
    );
    lines.push(`LITH_DEBUG_SCAN_MAX=${parseInt(process.env.LITH_DEBUG_SCAN_MAX || '12', 10) || 0}`);
    lines.push(
        `LITH_DEBUG_SCAN_PAYLOAD_BYTES=${parseInt(process.env.LITH_DEBUG_SCAN_PAYLOAD_BYTES || '32', 10) || 0}`,
    );
    lines.push(`LITH_DEBUG_BITS=${formatBool(parseBool(process.env.LITH_DEBUG_BITS, false))}`);
    lines.push(
        `LITH_DEBUG_BITS_PER_LINE=${parseInt(process.env.LITH_DEBUG_BITS_PER_LINE || '128', 10) || 0}`,
    );
    lines.push(`LITH_DEBUG_BITS_MAX=${parseInt(process.env.LITH_DEBUG_BITS_MAX || '0', 10) || 0}`);
    lines.push(
        `LITH_HAS_MORE_FLAG=${formatBool(parseBool(process.env.LITH_HAS_MORE_FLAG, false))}`,
    );
    lines.push(
        `FORCE_LOGIN_ON_FIRST_LITH=${formatBool(parseBool(process.env.FORCE_LOGIN_ON_FIRST_LITH, false))}`,
    );
    ...
    lines.push(
        `LOGIN_REQUIRE_CREDENTIALS=${formatBool(parseBool(process.env.LOGIN_REQUIRE_CREDENTIALS, false))}`,
    );
    lines.push(
        `LOGIN_RESEND_DUPLICATE_6D=${formatBool(
            parseBool(process.env.LOGIN_RESEND_DUPLICATE_6D, true),
        )}`,
    );
    lines.push(
        `LOGIN_RESPONSE_MINIMAL=${formatBool(parseBool(process.env.LOGIN_RESPONSE_MINIMAL, false))}`,
    );
    lines.push(
        `LOGIN_RESPONSE_TIMESTAMP=${formatBool(parseBool(process.env.LOGIN_RESPONSE_TIMESTAMP, false))}`,
    );
    lines.push('');
    lines.push('[world]');
    lines.push(
        `WORLD_SEND_PROTOCOL=${formatBool(parseBool(process.env.WORLD_SEND_PROTOCOL, false))}`,
    );
    lines.push(`WORLD_SEND_ID=${formatBool(parseBool(process.env.WORLD_SEND_ID, false))}`);
    lines.push(`FOM_WORLD_ID=${worldSelectWorldId}`);
    lines.push(`FOM_WORLD_INST=${worldSelectWorldInst}`);
    lines.push(`FOM_PLAYER_ID=${worldSelectPlayerId}`);
    lines.push(`FOM_PLAYER_ID_RANDOM=${formatBool(worldSelectPlayerIdRandom)}`);
    lines.push(`FOM_WORLD_CONST=0x${worldLoginWorldConst.toString(16)}`);
    lines.push('');
    lines.push('[files]');
    lines.push(
        `FOM_FILE_LIST=${formatBool(
            parseBool(process.env.FOM_FILE_LIST ?? process.env.WORLD_SEND_FILE_LIST, false),
        )}`,
    );
    lines.push(
        `FOM_FILE_LIST_MAX_BYTES=${parseInt(
            process.env.FOM_FILE_LIST_MAX_BYTES || '1024',
            10,
        ) || 1024}`,
    );
    lines.push(`FOM_FILE_ROOT=${process.env.FOM_FILE_ROOT || ''}`);
    lines.push(`FOM_FILE_ID_MAP=${process.env.FOM_FILE_ID_MAP || ''}`);
    lines.push('');
    lines.push('[raknet]');
    ...
    // eslint-disable-next-line no-console
    console.log(`[FoMServer] Wrote config snapshot to ${outPath}`);
}
