...
Common examples:
```
# Mirror the input tree into .\Open-FoMTools\Output\Models_LTA
python Open-FoMTools\ltb_to_lta.py .\Open-FoMEdit\Models --out-root .\Open-FoMTools\Output\Models_LTA

# Write .lta beside each .ltb (omit --out-root)
python Open-FoMTools\ltb_to_lta.py .\Open-FoMEdit\Models
```
...
2) Copy the resulting `.lta` to `Open-FoMEdit\Models` or your DEdit project model path.
