...
| 0x0049B760 | 0x0009B760 | Packet_ID_LOGIN_REQUEST_RETURN_Read | Reads optional 0x19 timestamp, then `success:u8c` + `session_str` (Huffman, <=2048). Bit order LSB (RakNet BitStream). NOTE: current Client IDB shows stub-like body (no BitStream read); re-verify | decomp + disasm | high |
