...
### Render / D3D9 init (FoTD build; image base 0x008D0000)
...
### Item templates (FoTD base 0x00400000)
| VA | RVA | Symbol | Purpose | Evidence | Conf |
|---|---|---|---|---|---|
...
| 0x10199A40 | 0x00199A40 | ClientShell_OnMessage_DispatchPacketId | CShell OnMessage packet-id switch (signed char); dispatches IDs like 0x6B/0x7B/0x7D and inventory packets | decomp | high |
| 0x1018E1F0 | 0x0018E1F0 | HandlePacket_ID_LOGIN_REQUEST_RETURN_6D | Login response handler (packet ID 0x6D) | dispatch + decomp | high |
...
| 0x1018DA20 | 0x0018DA20 | HandlePacket_ID_70 | Packet ID 0x70; parses bit + 32-byte buffer; if flag set updates Login UI via sub_10089620/UiText_SetValueIfChanged | decomp | low |
...
| 0x102404E0 | 0x002404E0 | ItemsAdded_payload_read | Type=2 payload for 0x7B; reads item list | decomp | med |
...
Read: Packet_ID_LOGIN_REQUEST_RETURN_Read @ 0x1018DCE0; handler: HandlePacket_ID_LOGIN_REQUEST_RETURN_6D @ 0x1018E1F0.
Fields (read order):
...
#### Packet_ID_70 (ID 0x70) - server->client (login UI)
Read/handler: HandlePacket_ID_70 @ 0x1018DA20; parse via sub_1008AA10.
Fields (read order, inferred):
- bit flag @+0x430; if set, BitStream_ReadBit_u8 into +0x431 then read 32 bytes into +0x452; else read 32 bytes into +0x432.
Handler behavior:
...
- type=2 -> ItemsAdded_payload_read @ 0x102404E0 into +0x438.
...
| 0x00499730 | 0x00099730 | Packet_ID_LOGIN_REQUEST_Ctor | Sets packet id 0x6C; clears fields | decomp | high |
| 0x0049A7F0 | 0x0009A7F0 | Packet_WriteHeader_OptionalTimestamp | Writes optional 0x19 timestamp + packet id byte from +0x428 (used by 0x6C/0x6E) | decomp + disasm | high |
| 0x0049B720 | 0x0009B720 | Packet_ID_LOGIN_REQUEST_Serialize | Writes Huffman username + u16 token (byte-swap gated by ShouldByteSwap) | decomp | high |
| 0x0049B3A0 | 0x0009B3A0 | Packet_ID_LOGIN_REQUEST_WriteTokenU16 | Writes 16-bit token via bitstream; byte-swap if ShouldByteSwap (0x004E3620) | decomp | med |
| 0x0049D090 | 0x0009D090 | LoginButton_OnClick | Builds Packet_ID_LOGIN_REQUEST and sends to master | decomp | high |
| 0x0049D250 | 0x0009D250 | ClientNetworking_DispatchPacket | Master/world packet dispatcher; routes 0x6D to ClientNetworking_HandleLoginRequestReturn_6D; handles addr updates (0x0E/0x13/0x14) and closes on 0x0F/10/12/15/16/17/5F | decomp + disasm | high |
| 0x0049B760 | 0x0009B760 | Packet_ID_LOGIN_REQUEST_RETURN_Read | Reads optional 0x19 timestamp, then `success:u8c` + `session_str` (Huffman, <=2048). Bit order LSB (RakNet BitStream). NOTE: current FoTD IDB shows stub-like body (no BitStream read); re-verify | decomp + disasm | high |
| 0x0049CA70 | 0x0009CA70 | ClientNetworking_HandleLoginRequestReturn_6D | Handles 0x6D success; builds session_md5 = MD5(MD5(this+0xD1) + session_str), fills login auth packet (username/masterHost/computer/MAC + optional blob), sends 0x6E | decomp | high |
...
| 0x0049B820 | 0x0009B820 | Packet_ID_LOGIN_Serialize | Serializes post-6D login auth packet (Huffman/bounded strings, optional blob raw bytes + blob_u32 with endian swap) | decomp | high |
...
### Packet_ID_LOGIN (0x6E) layout (FoTD client)
...
### ClientNetworking login context (FoTD client)
...
## FoTD (CShell.dll, image base 0x10000000)

### Item stats / tooltip pipeline (FoTD)
| VA | RVA | Symbol | Purpose | Evidence | Conf |
|---|---|---|---|---|---|
...
### Misc tables / personality helpers (FoTD / CShell)
| VA | RVA | Symbol | Purpose | Evidence | Conf |
|---|---|---|---|---|---|
...
### Weapon fire / recoil (FoTD / CShell)
| VA | RVA | Symbol | Purpose | Evidence | Conf |
|---|---|---|---|---|---|
...
### Weapon fire entry snapshot (FoTD / CShell)
| VA | RVA | Symbol | Purpose | Evidence | Conf |
|---|---|---|---|---|---|
...
### Deployable packets (FoTD / CShell)
| VA | RVA | Symbol | Purpose | Evidence | Conf |
|---|---|---|---|---|---|
...
### Base window helpers (FoTD / CShell)
| VA | RVA | Symbol | Purpose | Evidence | Conf |
|---|---|---|---|---|---|
...
### Inventory / stats glue (FoTD / CShell)
| VA | RVA | Symbol | Purpose | Evidence | Conf |
|---|---|---|---|---|---|
...
### Interface resources (FoTD / CShell)
| VA | RVA | Symbol | Purpose | Evidence | Conf |
|---|---|---|---|---|---|
...
### Message read helpers (FoTD / CShell)
| VA | RVA | Symbol | Purpose | Evidence | Conf |
|---|---|---|---|---|---|
...
### Compression helpers (FoTD / fom_client.exe)
| VA | RVA | Symbol | Purpose | Evidence | Conf |
|---|---|---|---|---|---|
...
### Player stats storage / StatGroup shared memory (FoTD)
| VA | RVA | Symbol | Purpose | Evidence | Conf |
|---|---|---|---|---|---|
...
### Observed player stat IDs (FoTD, inferred by usage)
...
### SharedMem flag indices (FoTD, observed usage)
...
### Playerfile packet parsing (FoTD)
| VA | RVA | Symbol | Purpose | Evidence | Conf |
|---|---|---|---|---|---|
