...
function resolveRuntimeTablePath(): string | null {
  ...
  candidates.push(path.resolve(process.cwd(), 'ClientEmulator', 'huffman_table_runtime.json'));
...
export function writeHuffmanStringRawLenBE(stream: RakBitStream, value: string, maxLen: number = 2048): void {
  const temp = encodeStringToBitStream(value ?? '', maxLen);
  const bitCount = temp.bits();
