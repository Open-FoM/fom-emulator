...
export class PacketLogger {
  ...
  constructor(options: { console?: boolean; file?: boolean; logDir?: string; consoleMode?: 'off' | 'summary' | 'full'; consoleMinIntervalMs?: number; consolePacketIds?: number[]; filePacketIds?: number[]; ignorePacketIds?: number[]; analysis?: boolean; consoleRepeatSuppressMs?: number; flushMode?: 'off' | 'login' | 'always' } = {}) {
    ...
    this.filePacketIds = options.filePacketIds && options.filePacketIds.length > 0 ? new Set(options.filePacketIds) : null;
    this.ignorePacketIds = options.ignorePacketIds && options.ignorePacketIds.length > 0 ? new Set(options.ignorePacketIds) : null;
  ...
  private initLogFile(): void {
    ...
    const latestName = path.join(this.logDir, 'fom_client.log');
    ...
    this.safeConsoleLog(`[PacketLogger] Logging to ${latestName}`);
  }
  ...
  private static installProcessHooks(): void {
    ...
    const disableConsole = (): void => {
      PacketLogger.globalInstance?.disableConsoleLogging();
    };
    ...
    if (process.stdout) {
      process.stdout.on('error', (err: NodeJS.ErrnoException) => {
        ...
        logEvent('stdout error', err, false);
      });
    }
    if (process.stderr) {
      process.stderr.on('error', (err: NodeJS.ErrnoException) => {
        ...
        logEvent('stderr error', err, false);
      });
    }
  }
  ...
  private logProcessEvent(label: string, detail?: unknown, fatal: boolean = false): void {
    ...
    if (fatal && this.logToConsole) {
      this.safeConsoleError(line);
      if (detail instanceof Error && detail.stack) {
        this.safeConsoleError(detail.stack);
      }
    }
  }
  ...
  log(packet: LoggedPacket): boolean {
    ...
    if (consoleDecision.log) {
      if (consoleDecision.suppressed > 0) {
        this.safeConsoleLog(`[PacketLogger] Suppressed ${consoleDecision.suppressed} repeats of ${consoleDecision.signature}`);
      }
  ...
  private logToConsoleFormatted(packet: LoggedPacket): void {
    ...
    this.safeConsoleLog(this.hexDumpColored(packet.data));
  }
  ...
  private logToConsoleSummary(packet: LoggedPacket): void {
    ...
    this.safeConsoleLog(`[${this.packetCount}] ${dir} ${addr}${connId} (${packet.data.length} bytes)${firstByte}`);
  }
  ...
  private shouldLogConsole(packet: LoggedPacket): { log: boolean; suppressed: number; signature: string } {
    ...
    const forceLogin = this.containsLoginMarker(packet.data) || effectiveId === 0x6b || effectiveId === 0x6d || effectiveId === 0x6e || effectiveId === 0x6f || effectiveId === 0x70;
    ...
    if (this.consolePacketIds) {
      ...
      if (!this.consolePacketIds.has(effectiveId)) return { log: false, suppressed: 0, signature };
    }
    if (forceLogin) {
  ...
  private shouldLogFile(packet: LoggedPacket): boolean {
    ...
    const effectiveId = this.getEffectiveId(packet);
    if (this.ignorePacketIds && effectiveId !== null && this.ignorePacketIds.has(effectiveId)) return false;
  ...
  private containsLoginMarker(buffer: Buffer): boolean {
    for (let i = 0; i < buffer.length; i += 1) {
      const b = buffer[i];
      if (b === 0x6b || b === 0x6d || b === 0x6e || b === 0x6f || b === 0x70) return true;
    }
    return false;
  }
  ...
  private consoleSignature(packet: LoggedPacket): string {
    ...
    const effectiveId = this.getEffectiveId(packet);
    const idTag = effectiveId !== null ? `0x${effectiveId.toString(16).padStart(2, '0')}` : 'none';
  ...
  logAnalysis(packet: LoggedPacket, consoleLogged: boolean = true): void {
    ...
    if (analysis.length > 0 && this.logToConsole) {
      this.safeConsoleLog(analysis.join('\n'));
    }
  }
